...
var Vector2D = (function () {
    ...
    Vector2D.prototype.multiply = function (that) {
        return this.x * that.x + this.y * that.y;
    };
    /**
     * returns NaN for that.length()= 0
     */
    Vector2D.prototype.angle = function (that) {
        return Math.acos((this.multiply(that)) / (this.length() * that.length()));
    };
    return Vector2D;
}());
// set some defaults
var originVec = new Vector2D(0, 50); // origin vector
var radius = 50; // the maxium disctance a pixle can have to be coloed black
var lineNo = 0;
...
lineReader.on('line', function (line) {
    if (lineNo > 0) {
        ...
        var r = line.split(' ');
        var P = Number(r[0]);
        var candidate = new Vector2D(Number(r[1]) - 50, Number(r[2]) - 50); // transform coordinates to the center of the square (50,50)
        var angle = ((P < 50 ? 0 : Math.PI) + candidate.angle(originVec)) / (2 * Math.PI) * 100;
